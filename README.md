...
wb_obj = openpyxl.load_workbook('meta_data.xlsx')
...
os.makedirs('3n', exist_ok=True)
os.makedirs('4n', exist_ok=True)
...
while i <= sheet_obj.max_row:
    ...
    if cell_value in ['3n', '4n']:
        ...
        if k < 10:
            source_path = f'Dataset\\{l+1}-{l+10}\\{l+1}-{l+10}\\XRAY_L2_T0{k}_{m}0{n}.bmp'
            target_path = f'Dataset\\3n\\XRAY_L2_T0{k}_{m}0{n}.bmp'
        else:
            source_path = f'Dataset\\{l+1}-{l+10}\\{l+1}-{l+10}\\XRAY_L2_T{k}_{m}0{n}.bmp'
            target_path = f'Dataset\\4n\\XRAY_L2_T{k}_{m}0{n}.bmp'
...
while i <= sheet_obj.max_row:
    ...
    if k < 10 :
        ...
        if  (not ( os.path.exists(f'Deleted 3n\\XRAY_L2_T0{k}_{m}0{n}.bmp' )
             or os.path.exists(f'Deleted 4n\\XRAY_L2_T0{k}_{m}0{n}.bmp'))):
                sheet_obj.delete_rows(i)
        else:
            i+=1
       
    else:
        if (not ( os.path.exists(f'Deleted 3n\\XRAY_L2_T{k}_{m}0{n}.bmp') 
             or os.path.exists(f'Deleted 4n\\XRAY_L2_T{k}_{m}0{n}.bmp'))):
                sheet_obj.delete_rows(i)
        else:
            i+=1
    print("i= ",i)  
